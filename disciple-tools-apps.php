<?php
/**
 * Plugin Name: Disciple.Tools - Disciple Tools Apps
 * Plugin URI: https://github.com/DiscipleTools/disciple-tools-apps
 * Description: Disciple.Tools - Disciple Tools Apps is intended to help developers and integrator jumpstart their extension of the Disciple.Tools system.
 * Text Domain: disciple-tools-apps
 * Domain Path: /languages
 * Version:  0.1
 * Author URI: https://github.com/DiscipleTools
 * GitHub Plugin URI: https://github.com/DiscipleTools/disciple-tools-apps
 ...
 * 1. @todo Rename the `disciple-tools-apps.php file.
 * 2. @todo Refactor all occurrences of the name Disciple_Tools_Apps, disciple_tools_apps, disciple-tools-apps, apps, and "Disciple Tools Apps"
...
/**
 * Gets the instance of the `Disciple_Tools_Apps` class.
 *
 * @since  0.1
 * @access public
 * @return object|bool
 */
function disciple_tools_apps() {
    $disciple_tools_apps_required_dt_theme_version = '1.19';
    ...
    if ( $is_theme_dt && version_compare( $version, $disciple_tools_apps_required_dt_theme_version, "<" ) ) {
        add_action( 'admin_notices', 'disciple_tools_apps_hook_admin_notice' );
    ...
    return Disciple_Tools_Apps::instance();

}
add_action( 'after_setup_theme', 'disciple_tools_apps', 20 );
...
class Disciple_Tools_Apps {
    ...
    private function __construct() {
         ...
         */
        if ( $is_rest && strpos( dt_get_url_path(), 'disciple-tools-apps' ) !== false ) {
         ...
         */
        if ( strpos( dt_get_url_path(), 'metrics' ) !== false || ( $is_rest && strpos( dt_get_url_path(), 'disciple-tools-apps-metrics' ) !== false ) ){
    ...
    public static function deactivation() {
        ...
        delete_option( 'dismissed-disciple-tools-apps' );
    }
    ...
    public function __call( $method = '', $args = array() ) {
        _doing_it_wrong( "disciple_tools_apps::" . esc_html( $method ), 'Method does not exist.', '0.1' );
        unset( $method, $args );
        return null;
    }
}
...
register_activation_hook( __FILE__, [ 'Disciple_Tools_Apps', 'activation' ] );
register_deactivation_hook( __FILE__, [ 'Disciple_Tools_Apps', 'deactivation' ] );


if ( ! function_exists( 'disciple_tools_apps_hook_admin_notice' ) ) {
    function disciple_tools_apps_hook_admin_notice() {
        global $disciple_tools_apps_required_dt_theme_version;
        $wp_theme = wp_get_theme();
        $current_version = $wp_theme->version;
        $message = "'Disciple.Tools - Disciple Tools Apps' plugin requires 'Disciple.Tools' theme to work. Please activate 'Disciple.Tools' theme or make sure it is latest version.";
        if ( $wp_theme->get_template() === "disciple-tools-theme" ){
            $message .= ' ' . sprintf( esc_html( 'Current Disciple.Tools version: %1$s, required version: %2$s' ), esc_html( $current_version ), esc_html( $disciple_tools_apps_required_dt_theme_version ) );
        }
        // Check if it's been dismissed...
        if ( ! get_option( 'dismissed-disciple-tools-apps', false ) ) { ?>
            <div class="notice notice-error notice-disciple-tools-apps is-dismissible" data-notice="disciple-tools-apps">
                <p><?php echo esc_html( $message );?></p>
            </div>
            <script>
                jQuery(function($) {
                    $( document ).on( 'click', '.notice-disciple-tools-apps .notice-dismiss', function () {
                        $.ajax( ajaxurl, {
                            type: 'POST',
                            data: {
                                action: 'dismissed_notice_handler',
                                type: 'disciple-tools-apps',
 ...
 * @link https://github.com/DiscipleTools/disciple-tools-apps/wiki/Configuring-Remote-Updating-System
 *
...
//                'https://raw.githubusercontent.com/DiscipleTools/disciple-tools-apps/master/version-control.json',
//                __FILE__,
//                'disciple-tools-apps'
//            );
//
